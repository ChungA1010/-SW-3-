"""BEATs-based multi-axis intensity model."""
...
class BEATsMultiAxisIntensity(nn.Module):
    def __init__(self, pretrained_checkpoint_path: str | Path | None = None, backbone_config_override: dict | None = None) -> None:
        super().__init__()
        ...
        self.drive_head = nn.Linear(256, 5)
        self.phase_head = nn.Linear(256, 5)
        self.space_head = nn.Linear(256, 5)

    def forward(self, waveforms: torch.Tensor, padding_mask: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        return self.drive_head(features), self.phase_head(features), self.space_head(features)
